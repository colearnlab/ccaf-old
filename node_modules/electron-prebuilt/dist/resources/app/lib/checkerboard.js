...
function recursiveStringReplace(obj) {
  if (!isPOJS(obj))
    return stringReplace(obj);

  if (obj instanceof Array)
    obj = obj.map(function(item) {
      return recursiveStringReplace(item);
    });
  else
    for (var prop in obj)
      obj[prop] = recursiveStringReplace(obj[prop]);

  return obj;
}

function recursiveUnStringReplace(obj) {
  if (!isPOJS(obj))
    return unStringReplace(obj);

  if (obj instanceof Array)
    obj = obj.map(function(item) {
      return recursiveUnStringReplace(item);
    });
  else
    for (var prop in obj)
      obj[prop] = recursiveUnStringReplace(obj[prop]);

  return obj;
}

// functions have some built in properties that cannot be overwritten. in that case, we add the null
// character to the end of the prop name. thus it can be called state['prop\0']. this is messy but it
// ensures that no other property names are overwritten (unless you are evil and typically end your
// property names with the null character)
var reserved = ['arguments', 'caller', 'displayName', 'length', 'name', 'prototype', 'apply', 'bind', 'call',
  'isGenerator', 'toSource', 'toString'];
function safetyProp(prop) {
...
function DiffableStateFactory(root, prop, data) {
  ...
  function State(newValorProp, newVal) {
    ...
    if (arguments.length === 1) {
      ...
      State.$$.root[safetyProp(State.$$.prop)] = DiffableStateFactory(State.$$.root, State.$$.prop, newValorProp);
      State.$$.root[safetyProp(State.$$.prop)].$$.patched = true;
      State.$$.root.$$.patch[State.$$.prop] = recursiveStringReplace(newValorProp);
      return newValorProp;
    }

    if (arguments.length === 2) {
      if (newValorProp in State.$$.data)
        State[safetyProp(newValorProp)](newVal);
      else {
        State.$$.data[newValorProp] = newVal;
        State[safetyProp(newValorProp)] = DiffableStateFactory(State, newValorProp, undefined);
        State[safetyProp(newValorProp)](newVal);
      }
    }
  }
  ...
  function itr(merge, indexOrProp) {
    if (isPOJS(State.$$.data[indexOrProp]) || State.$$.data[indexOrProp] instanceof Array)
      merge[indexOrProp] = recursiveUnStringReplace(State[indexOrProp].$$.merge());
    else if (typeof State.$$.root.$$.patch[prop] !== 'undefined' && indexOrProp in State.$$.root.$$.patch[prop])
      merge[indexOrProp] = recursiveUnStringReplace(State.$$.root.$$.patch[prop][indexOrProp]);
    else
      merge[indexOrProp] = recursiveUnStringReplace(State.$$.data[indexOrProp]);
  }
  ...
  if (data instanceof Array)
    data.forEach(function(item, index) {
      State[safetyProp(index)] = DiffableStateFactory(State, index, data[index]);
    });
  else if (typeof data === 'object')
    for (var _prop in data)
      if (data.hasOwnProperty(_prop))
        State[safetyProp(_prop)] = DiffableStateFactory(State, _prop, data[_prop]);
...
function SharedTransactionalMemory(conn) {
  ...
  function updateState(newState) {
    ...
    else if (!waitingForReturn && 'onchange' in stm && typeof stm.onchange === 'function') stm.onchange(new DiffableStateFactory(null, 'root', state));
  }
  ...
  conn.onmessage = function(json) {
    ...
    if (typeof envelope.message.uuid !== 'undefined') uuid = envelope.message.uuid;
