(function(){
  ...
  function notifyAdminStateChange(classroom) {
    conns.forEach(function(otherConn) {
      if (assoc[otherConn.uuid].classroom === classroom._id && typeof assoc[otherConn.uuid]._id === 'undefined')
    ...
    'data-attempt-state': function(conn, message) {
      ...
      withClassroom(assoc[conn.uuid].classroom, function(err, classroom) {
        ...
        if (typeof lastAttempt !== 'undefined')
          conns.forEach(function(otherConn) {
            if (typeof assoc[otherConn.uuid] !== 'undefined' && assoc[otherConn.uuid].classroom === assoc[conn.uuid].classroom) {
              // don't notify self.
              if (otherConn == conn)
                return;
  ...
  module.exports.start = function(port) {
    ...
    WebSocketServer.on('connection', function(conn) {
      ...
      conn.on('close', function() {
        ...
        if (typeof assoc[conn.uuid] !== 'undefined' && typeof assoc[conn.uuid]._id !== 'undefined')
          withClassroom(assoc[conn.uuid].classroom, function(err, classroom) {
            classroom.getDevice(assoc[conn.uuid]._id).connected = false;
            classroom.save();
            notifyAdminStateChange(classroom);
          });
