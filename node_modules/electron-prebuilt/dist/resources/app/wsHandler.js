    ...
    WebSocketServer.on('connection', function(conn) {
      ...
      conn.on('close', function() {
        ...
        for (var prop in assoc) {
          if (assoc[prop].classroom === savedAssoc.classroom &&
            typeof assoc[prop]._id !== 'undefined' &&
            assoc[prop]._id === savedAssoc._id &&
            assoc[prop] !== savedAssoc) {
              disconnect = false;
            }
        }

        if (typeof savedAssoc !== 'undefined' && typeof savedAssoc._id !== 'undefined' && disconnect)
          withClassroom(savedAssoc.classroom, function(err, classroom) {
            var device = classroom.getDevice(savedAssoc._id);
