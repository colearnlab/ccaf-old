...
function mergeClient(classroomId, index, mergeState) {
  db.find({'_id': classroomId}, function(err, docs) {
    if (docs.length === 0) return;
    var classroom = docs[0];
    var device = classroom.devices[classroom.devices.map(function(device) { return device._id; }).indexOf(parseInt(index))];
    for (var prop in mergeState)
      device[prop] = mergeState[prop];
    db.update({'_id': classroomId}, classroom);
    wsAdmin.sendAll('event-client-updated', {'classroom': classroomId, 'device': device});
  });
}
...
var wsClientMessageHandler = {
  'event-set-active': function(wsConn, message, flags) {
    ...
    mergeClient(wsConn.classroom, wsConn.index, {'connected': true});
    wsConn.sendObj('event-acknowledge-active');
  },
  'event-close': function(wsConn, message, flags) {
    mergeClient(wsConn.classroom, wsConn.index, {'connected': false});
  },
  'data-client-state': function(wsConn, message, flags) {
    mergeClient(wsConn.classroom, wsConn.index, message.state);
  }
};
