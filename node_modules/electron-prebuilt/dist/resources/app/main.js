...
global.isType = function isType(unknown, typesToCheck) {
  var knownType = type(unknown);
  for (var i = 0; i < arguments.length; i++)
    if (knownType == arguments[i])
      return true;

  return false;
};

type.number = 0; type.string = 1; type.array = 2; type.object = 3;
  type.function = 4; type.undefined = 5; type.null = 6;
...
var wsClientMessageHandler = {
  'event-set-active': function(wsConn, message, flags) {
    assert(type(message.classroom) != type.undefined, 'bad classroom id');
    assert(type(message._id) != type.undefined, 'bad device id');

    // Add relevant details to the connection object for later lookup on send/recv
    wsConn.classroom = message.classroom;
    wsConn._id = message._id;

    withClassroom(message.classroom, function(err, classroom) {
      var device = classroom.getDevice(message._id, {'connected': true});
      classroom.getDevice(message._id).sendObj('event-acknowledge-active');
      classroom.save(function(err, classroom) {
        if (type(err) == type.null)
          wsAdmin.sendAll('event-client-updated', {'classroom': wsConn.classroom, 'device': device});
        else
          assert(false, err);
      });
  ...
  'event-close': function(wsConn, message, flags) {
    if (type(wsConn.classroom) == type.undefined)
      return;
    withClassroom(wsConn.classroom, function(err, classroom) {
      var device = classroom.getDevice(wsConn._id, {'connected': false});
      classroom.save(function(err, classroom) {
        if (type(err) == type.null)
          wsAdmin.sendAll('event-client-updated', {'classroom': wsConn.classroom, 'device': device});
        else
...
app.on('window-all-closed', function() {
  db.find({}, function(err, classrooms) {
    classrooms.forEach(function(classroom) {
      if (type(classroom.devices) == type.array) {
        classroom.devices.forEach(function(device) {
          device.connected = false;
        });
        db.update({'_id': classroom._id}, {'$set': classroom});
      }
    });
    wsAdmin.sendAll('event-server-disconnected');
    wsClient.sendAll('event-server-disconnected');
  });
