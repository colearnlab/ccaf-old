...
function Classroom(args) {
  ...
  this.deviceIndex = args.deviceIndex || 0;
}
  ...
  state.classrooms.forEach(function(classroom, index) {
    ...
    classroom.devices.forEach(function(device, index) {
      ...
      if (typeof device.id === 'undefined')
        device.id = classroom.deviceIndex++;
...
checkerboard.on('open', function(conn, message) {
  ...
  conn.state = function(state) {
    ...
    state.classrooms.forEach(function(classroom) {
      toReturn.classrooms.push({'id': classroom.id, 'name': classroom.name, 'devices': classroom.devices});
    });
...
checkerboard.on('data-associate', function(conn, message) {
  ...
  conn.state = function(state) {
    ...
    if (typeof message.device !== 'undefined') {
      var device = classroom.devices[classroom.devices.map(function(d) { return d.id; }).indexOf(parseInt(message.device))];
      if (typeof device === 'undefined')
        return {};
      return {'device': device, 'global': typeof device.app !== 'undefined' ? classroom.appRoot[device.app] : {}};
    }
    else
      return classroom;
  };
