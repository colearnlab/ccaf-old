  ...
  var Home = {
    view: function(ctrl) {
      var w = app.activeClassroom().dimensions.width(),
          ...
          accentWall = app.activeClassroom().accentWall;
    ...
    view: function(ctrl) {
      ...
      return (
        m('div.deviceTile' + (device.connected() ? '.deviceActive' : ''), {
          ...
          m('div.deviceName', device.name())
        ])
      );
    }
  };

  // todo: refactor into two/three components:
  // button
  //    - switch to edit on device drop-target
  // form
  //    - edits or deletes
  var AddDevice = {
    controller: function(args) {
      ...
      var addDevice = function() {
        deviceToAdd().save().then(function(device) {
          app.activeClassroom().devices.push();
        });
        deviceToAdd(null);
      };
      return {
    ...
    view: function(ctrl) {
      ...
      if (ctrl.deviceToAdd() === null)
        toggle = (
          m('button.btn.btn-lg.btn-success.addButton', {
            'onclick': function(e) {
              var deviceDimensions = parseInt(app.activeClassroom().dimensions.width() / 25);
              ctrl.deviceToAdd(new DeviceModel({
                'location': {'x': 0, 'y': 0},
                'screen': {
                  'width': deviceDimensions || 1,
