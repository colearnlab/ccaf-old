var ClassroomModel = function(args) {
  ...
  this.accentWall = m.prop({
    'color': m.prop(args.accentWall.color),
    'location': m.prop(args.accentWall.location)
  });

  this.devices = m.prop(args.devices.map(function(device) {
...
ClassroomModel.get = ClassroomModel.prototype.get = function(args) {
  return m.request({
    'method': 'GET',
    'url': '/api/classroom/' + (args && args._id ? args._id() : ''),
    'type': ClassroomModel
  });
};
...
ClassroomModel.save = ClassroomModel.prototype.save = function(data) {
  ...
  return m.request({
    ...
    'url': '/api/classroom/' + (typeof data._id() !== 'undefined' ? data._id() : ''),
    'data': data,
    'type': ClassroomModel
  });
};
...
DeviceModel.get = DeviceModel.prototype.get = function(args) {
  return m.request({
    'method': 'GET',
    'url': '/api/classroom/' + app.activeClassroom()._id() + '/device/' + (args && args._id ? args._id() : ''),
    'type': DeviceModel
  });
};
...
DeviceModel.save = DeviceModel.prototype.save = function(data) {
  ...
  return m.request({
    'method': (typeof data._id() !== 'undefined' ? 'PUT' : 'POST'),
    'url': '/api/classroom/' + app.activeClassroom()._id() + '/device/' + (typeof data._id() !== 'undefined' ? data._id() : ''),
    'data': data,
    'type': DeviceModel
  });
};
