define(['clientUtil'], function(clientUtil) {
 ...
 addToPath = function(path, x, y) {
    ...
    canvas.ontouchstart = function(e) {
      [].forEach.call(e.changedTouches, function(ct) {
        ...
        appRoot.try(function(root) {
          root.deviceState[params.device].drawings[root.deviceState[params.device].version][touchToPath[ct.identifier].id] = JSON.parse(JSON.stringify(touchToPath[ct.identifier]));
        }, function(root) {
          var tmp = appRoot.subscribe('deviceState.' + params.device + '.drawings.' + root.deviceState[params.device].version + touchToPath[ct.identifier].id, undefined, function() {
  ...
  var version;
  var drawnByMe = {};
  function update(_paths) {
    for (var prop in _paths) {
      if (prop in drawnByMe || (prop in paths && paths[prop].strokeFinished === true))
        continue;
      if (prop in paths && !paths[prop].strokeFinished) {
        paths[prop].X = _paths[prop].X;
        paths[prop].Y = _paths[prop].Y;
      } else if (!(prop in paths)) {
        paths[prop] = PathFactory(_paths[prop]);
      }
      drawPath(paths[prop]);
    }
  }
