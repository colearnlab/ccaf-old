...
Client.prototype.processCursorAlive = function(packet, message){
  ...
  var activeCursors = message.sessionIds.map(function(id){ return id.toString(); });
  var notActiveCursors = [];
  for (var prop in this.cursors[packet.source])
    if (!(prop in activeCursors))
        notActiveCursors.push(this.cursors[packet.source][prop]);
...
Client.prototype.createTouchEvent = function(type, touch){
  ...
  // do not support it.
  var touchEvent = new Event(type, {'bubbles': true, 'cancelable': false});
